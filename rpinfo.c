...
print_temp (char temp_type)
{
  ...
  if (!fp)
    {
      snprintf (error, sizeof (error),
		"Error reading temperature. Error opening file [ %s ]",
		RPI_TEMP_FILE);
      perror (error);
      return (EXIT_FAILURE);
    }
  ...
  if (ferror (fp))
    {
      snprintf (error, sizeof (error),
		"Error reading temperature. Error reading file [ %s ]",
		RPI_TEMP_FILE);
      perror (error);
      return (EXIT_FAILURE);
    }
  ...
  if (temp == buf || *temp != '\0'
      || ((raw_temp == LONG_MIN || raw_temp == LONG_MAX) && errno == ERANGE))
    {
      fprintf (stderr,
	       "Error reading temperature: Could not convert '%s' to long and leftover string is: '%s'\n",
	       buf, temp);
      return (EXIT_FAILURE);
    }
 ...
 */
char *
get_cpuinfo_value (char *key, char *buf)
{
...
print_revision ()
{
  ...
  if (!fp)
    {
      snprintf (error, sizeof (error),
		"Error reading revision. Error opening file [ %s ]",
		RPI_CPUINFO_FILE);
      perror (error);
      return (EXIT_FAILURE);
    }
  ...
  if (ferror (fp))
    {
      snprintf (error, sizeof (error),
		"Error reading revision. Error reading file [ %s ]",
		RPI_CPUINFO_FILE);
      perror (error);
      return (EXIT_FAILURE);
    }
  ...
  if (revision == NULL)
    {
      fprintf (stderr,
	       "Error reading revision. Unable to find revision number (are you running from a real Raspberry PI?)\n");
      return (EXIT_FAILURE);
    }
  ...
  if (rpid == -1)
    {
      fprintf (stderr,
	       "Unable to find model information in revision database.  Please report your revision: %s\n",
	       revision);
      return (EXIT_FAILURE);
    }
